...
class CommentService extends Service
{
    ...
    public function removeTaskComment($comment_id, User $user)
    {
        ...
        if ($comment->user_id === $user->id || $comment->task_solution->tasker_id === $user->id) {
            $allCommentIds = $this->getAllDescendantIds($comment_id);

            // Đánh dấu tất cả là is_remove
            TaskComment::whereIn('id', $allCommentIds)->update(['is_remove' => true]);
            return true;
        }
        return false;
    }
    ...
    {
        $ids = TaskComment::where('parent_id', $parentId)->pluck('id')->toArray();
        foreach ($ids as $id) {
            $ids = array_merge($ids, $this->getAllDescendantIds($id));
        }
        return $ids;
    }
